...
- `src/wowprofit/service.py` – web use-cases: `MarketCache`; `search` over the selected realm/faction's characters (`engine.recipes_for_characters`: learned recipes, or with `include_unlearned` every recipe of their professions; chains only sub-craft through those); realm/faction `selection`; `sync`, which re-imports Alt Army characters and the selected realm's Auctionator prices whenever either file's mtime changes; game data update
- `src/wowprofit/api.py` – FastAPI JSON API (`create_app(db_path, ...)`); also serves the built `frontend/dist`. `GET /api/status` runs the addon sync, and the front end polls it (every 30 s and on window focus); its `data_version` is part of the characters/rank query keys
